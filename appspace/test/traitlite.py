...
def class_of(this):
    ...
    return add_article(this.__class__.__name__)
...
def getmembers(this, predicate=None):
    '''
...
class HasTraits(object):
        ...
        for c in callables:
            ...
            if callable(c):
                ...
                if nargs + offset == 0:
                    ...
                    raise TraitError(
                        'a trait changed callback must have 0-3 arguments.'
                    )
            else:
    ...
    def traits(self, **metadata):
        ...
        '''
        traits = dict([
            m for m in getmembers(self.__class__) if isinstance(m[1], TraitType)]
        )
        if len(metadata) == 0:
            return traits
        for meta_name, meta_eval in metadata.items():
...
class Instance(ClassBasedTraitType):
    ...
    def __init__(self, klass=None, args=None, kw=None, allow_none=True, **metadata):
        '''
        ...
        if any([
            klass is None,
            any([(not (inspect.isclass(klass), isinstance(klass, basestring)))])
        ]):
            raise TraitError(
                'The klass argument must be a class you gave: %r' % klass
            )
        self.klass = klass
...
class DottedObjectName(ObjectName):
    ...
    def validate(self, this, value):
        ...
        if all([
            ...
            all(self.isidentifier(x) for x in value.split('.'))
        ]):
            return value
        self.error(this, value)
...
class Container(Instance):
    ...
    def __init__(self, trait=None, default_value=None, allow_none=True, **metadata):
        '''
        ...
        **metadata : any
        ...
        if default_value is None:
            ...
            raise TypeError('default value of %s was %s' % (
                self.__class__.__name__, default_value
            ))
        if istrait(trait):
            self._trait = trait()
        ...
        super(Container, self).__init__(
            klass=self.klass, args=args, allow_none=allow_none, **metadata
        )
...
class Dict(Instance):
    ...
    def __init__(self, default_value=None, allow_none=True, **metadata):
        ...
        super(Dict, self).__init__(
            klass=dict, args=args, allow_none=allow_none, **metadata
        )
